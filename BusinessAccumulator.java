...
public class BusinessAccumulator implements ActionListener {
	...
	public JTextField inNumber = new JTextField(10); // sets the length of the
														// text field
	public JTextField tot = new JTextField(5); // sets the length of the text
												// field
	...
	public JLabel totLabel = new JLabel("Total ="); // Text right beside the
													// "tot" text field
	public JLabel inNumLabel = new JLabel("Enter amount to add =>"); // Text
																		// right
																		// beside
																		// the
																		// "inNumber"
	 ...
	 * 
	 * GUI Construction Current Status: 1 more issue GUI Construction Current
	 * Issues: Can't seem to figure out the scroll bar...Simple I know. ayyy
	 * lmao
	 * 
	 * GUI Function Current Status: Almost done. GUI Function Current Issues:
	 ***********************************************************************************
	 * 
	 */
	...
	public BusinessAccumulator() {
		...
		clrtotnum.add(tot); // | - This section sets the panel on the top of the
							// GUI
		...
		inNumber.addActionListener(this); // The reason that this is on the
											// inNumber TEXTFIELD is because the
											// actionListener by default
											// listens for the ENTER key. When
											// the enter key is pressed in the
											// inNumber textfield, an a
											// action will occur.
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (e.getSource() == inNumber) {
			...
			if ((CheckForUserInput(userInput))) {
				...
				// Continue on to accumulate the value and displaying it in the
				// "log" textArea
				// Alden's code already has set the value that will be used to
				// accumulate
				AccumulateTotal();
	 ...
	 * the new calculated value. Places it in the "log" textArea and scrolls to
	 * the bottom
	 * 
	 * Current status: ALMOST DONE Issues: Treats EVERY numerical input as a
	 * double. If the new accumulated value is nonnegative, then the "tot" text
	 * field background needs to be CYAN If the new accumulated valie is
	 * negative, then the "tot" text field background needs to be RED
	 ***********************************************************************************
	 * 
	 */
	...
	void AccumulateTotal() {
		...
		if (globalUserInputValueMath >= 0) {
			...
			// Shows up in "log" textArea as "old_value + typed_value =
			// new_value"
			...
			// oldAccumulate + " + " + globalUserInputValueMath + " = " +
			// accumulatedValue;
			...
			// Shows up in "log" textArea as "old_value -
			// absolute_value_of(typed_value) = new_value"
			// Check Bowman's program if unsure of "log" format

			// Set string newAccumulate to what it should look like here
			double absolute_userinput = Math.abs(globalUserInputValueMath);// takes
																			// absolute
																			// value
																			// of
																			// our
																			// globalUserInputValueMath
			absolute_userinput = -1 * (globalUserInputValueMath);// takes
																	// absolute
																	// value of
																	// our
																	// globalUserInputValueMath
			accumulatedValue = globalUserInputValueMath + accumulatedValue;// calculates
																			// new
																			// accumulated
																			// Value
			accumulatedValue = Math.round(accumulatedValue * 100.0) / 100.0;
	 ...
	 * 
	 * Checks for errors and returns false if the value to be accumulated has
	 * errors Sets a global variable to what the user inputed as a double
	 * 
	 * Status: ALMOST DONE Issue:
	 ***********************************************************************************
	 * 
	 */
	...
	private static boolean CheckForUserInput(String userInput) {
		...
		if (userInput.endsWith("d") || userInput.endsWith("f")) {
		...
		if (userInput.contains(".")) {
			...
			if ((userInput.length() - 3) == decimalLocation) {
				...
				// Trace Statement:System.out.println("truee");
			} else{
				// Trace Statement:System.out.println("problem");
			}
		}

		int commaCount = userInput.length() - userInput.replace(",", "").length();
		String checkCorrectAmountCommas = userInput;
		if( userInputContainsTwoAfterDecimal == 2){
			checkCorrectAmountCommas = checkCorrectAmountCommas.substring(0, checkCorrectAmountCommas.indexOf('.'));
			// Trace Statement:System.out.println("checkCorrectAmountCommas: " + checkCorrectAmountCommas);
			
			int counter = 0;
			for( int i=0; i<checkCorrectAmountCommas.length(); i++ ) {
			    if( checkCorrectAmountCommas.charAt(i) == ',' ) {
			        counter++;
			    } 
			}
			//If there is more than three digits per comma
			if((checkCorrectAmountCommas.length() / (counter + 1)) > 3 ){
				errorWithUserInput = ("Illegal use of comma in " + userInput);
				return false;
			}
		}
		...
		if (commaCount > 0) {
			if (userInput.contains(",")) {
				...
				if ((userInput.length() - correctLocationFactor) == 3) {
					userInputContainsThreeAfterComma = 2;
					// Trace Statements
					// Trace Statement:System.out.println("#####" + userInput.length() + "#####" + correctLocationFactor);

					// Trace Statement:System.out.println("no decimal and works");
				} else if ((userInput.length() - (correctLocationFactor)) == 6) {
					userInputContainsThreeAfterComma = 2;
					// Trace Statements
					// Trace Statement:System.out.println("#####" + userInput.length() + "#####" + correctLocationFactor);
			...
			while (commaCount > 1) {
				...
				if (previousCommaLocation == 0) {
					...
					if ((nextCommaLocation > 3) || (nextCommaLocation == 0)) {
						// Trace Statement:System.out.println("Entered more than three numbers in front of the comma");
						errorWithUserInput = ("Illegal use of comma in " + userInput);
						return false;
					...
					if (userInputContainsTwoAfterDecimal == 2) {
						...
						System.out.println("checkCommaPlacements: " + checkCommaPlacements);
					}
				}
